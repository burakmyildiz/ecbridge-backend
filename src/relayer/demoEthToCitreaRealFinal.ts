// Final fixed demo relayer with periodic pending transaction checks
...
const PENDING_CHECK_INTERVAL_MS = 30_000; // 30 seconds
...
async function handleDepositDemo(
    ...
    if (processingDeposits.has(nonceNumber)) {
        logger.warn(`🎬 DEMO: Deposit #${nonceNumber} already being processed`);
        return;
    }
    ...
    try {
        ...
        if (isAlreadyProcessed) {
            logger.info(`🎬 DEMO: Deposit #${nonceNumber} already processed on-chain.`);
        ...
        if (!transaction) {
            ...
            await transaction.save();
            publishTransactionUpdate(transaction);
            logger.info(`🎬 DEMO: Created new DB entry for transaction ${txHash}`);
        }
...
async function processPendingTransactions(): Promise<void> {
    ...
    try {
        ...
        logger.info(`🎬 DEMO: Found ${pendingTxs.length} pending transactions to check`);
            ...
            for (const log of receipt.logs) {
                try {
                    ...
                    if (parsed && (parsed.name === 'DepositETH' || parsed.name === 'DepositERC20')) {
                        const { nonce } = parsed.args;
                        const nonceNumber = Number(nonce);
                        
                        // Skip if already processing
                        if (processingDeposits.has(nonceNumber) || processedDeposits.has(nonceNumber)) {
                            continue;
                        }
                        
                        logger.info(`🎬 DEMO: Found pending deposit nonce ${nonce} from transaction ${tx.txHash}`);
...
export default async function startDemoRealRelayer(): Promise<void> {
    ...
    // Track the actual last block we've seen events up to
    let lastProcessedL1Block = await l1Provider.getBlockNumber() - L1_BLOCK_CONFIRMATIONS;
    let lastEventBlock = lastProcessedL1Block; // Track separately
    
    logger.info(`🎬 DEMO: Starting L1 scan from block number: ${lastProcessedL1Block}`);

    // Periodic check for missed pending transactions
    setInterval(async () => {
        logger.info(`🎬 DEMO: Periodic check for pending transactions...`);
        await processPendingTransactions();
    ...
    setInterval(async () => {
        try {
            ...
            const fromBlock = lastEventBlock + 1; // Use lastEventBlock instead
            const toBlock = head - L1_BLOCK_CONFIRMATIONS;

            if (fromBlock > toBlock) {
                // Don't update lastEventBlock here - only update when we actually scan
                return;
            }
            ...
            for (const event of allEvents) {
                ...
                logger.info(`🎬 DEMO: Found deposit nonce ${nonceNumber} in block ${event.blockNumber}`);
            ...
            // Only update lastEventBlock after successful scan
            lastEventBlock = toBlock;
            logger.info(`🎬 DEMO: Updated last scanned block to ${lastEventBlock}`);

        } catch (error: any) {
            logger.error(`🎬 DEMO: Error in main relayer loop: ${error.message}`);
            // Don't update lastEventBlock on error
        }
    }, POLLING_INTERVAL_MS);
}
